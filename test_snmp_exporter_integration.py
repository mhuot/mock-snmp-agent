...
class SNMPExporterIntegrationTester:
    ...
    def log_result(self, name: str, status: str, details: str, metrics: int = 0, duration: float = 0.0):
        """Log a test result"""
    ...
    def start_mock_agent(self) -> bool:
        ...
        try:
            ...
            for i in range(30):
                try:
                    response = requests.get(f"http://127.0.0.1:{self.api_port}/health", timeout=1)
    ...
    def start_snmp_exporter(self, config_file: str) -> bool:
        ...
        try:
            ...
            for i in range(30):
                try:
                    response = requests.get(f"http://127.0.0.1:{self.exporter_port}/metrics", timeout=1)
                    if response.status_code == 200:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if len(found_metrics) >= 2:  # At least snmp_up and one system metric
                    self.log_result("Basic Scraping", "PASS", 
                                  f"Successfully scraped system metrics", 
                                  len(metric_lines), time.time() - start_time)
                else:
                    self.log_result("Basic Scraping", "FAIL", 
            ...
            self.log_result("Basic Scraping", "FAIL", str(e), 0, time.time() - start_time)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if len(found_interface_metrics) >= 2:
                    self.log_result("Interface Metrics", "PASS", 
                                  f"Interface metrics collected: {found_interface_metrics}", 
                                  len(metric_lines), time.time() - start_time)
                else:
                    self.log_result("Interface Metrics", "FAIL", 
            ...
            self.log_result("Interface Metrics", "FAIL", str(e), 0, time.time() - start_time)
    ...
    def test_counter_progression(self) -> None:
        ...
        try:
            ...
            if config_response.status_code != 200:
                self.log_result("Counter Progression", "FAIL", 
            ...
            if len(counter_values) >= 2:
                ...
                is_progressing = any(counter_values[i] != counter_values[i-1] for i in range(1, len(counter_values)))
                
                if is_progressing:
                    self.log_result("Counter Progression", "PASS", 
                                  f"Counters progressing: {counter_values}", 
                                  len(counter_values), time.time() - start_time)
                else:
                    self.log_result("Counter Progression", "FAIL", 
            ...
            self.log_result("Counter Progression", "FAIL", str(e), 0, time.time() - start_time)
    ...
    def test_response_time_simulation(self) -> None:
        ...
        try:
            ...
            if config_response.status_code != 200:
                self.log_result("Response Time Simulation", "FAIL", 
            ...
            if response.status_code == 200 and scrape_duration > 1.0:
                self.log_result("Response Time Simulation", "PASS", 
            ...
            requests.post(f"http://127.0.0.1:{self.api_port}/api/v1/agents/configure", json=reset_config)
            
        except Exception as e:
            self.log_result("Response Time Simulation", "FAIL", str(e), 0, time.time() - start_time)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for line in metric_lines:
                    if 'ifIndex=' in line:
                        try:
                            ...
                            idx_part = [part for part in line.split(',') if 'ifIndex=' in part][0]
                ...
                if len(interface_indices) >= 2:
                    self.log_result("Bulk Operations", "PASS", 
                                  f"Bulk retrieval successful - {len(interface_indices)} interfaces", 
                                  len(metric_lines), time.time() - start_time)
                else:
                    self.log_result("Bulk Operations", "FAIL", 
            ...
            self.log_result("Bulk Operations", "FAIL", str(e), 0, time.time() - start_time)
