...
from .models import (
    HealthResponse, MetricsResponse, ConfigurationUpdate, ConfigurationResponse,
    AgentStatusResponse, RestartRequest, RestartResponse, ErrorResponse, OIDListResponse
)
...
class SNMPAgentAPIServer:
    ...
    def _setup_routes(self):
        ...
        @self.app.get("/config", response_model=ConfigurationResponse, tags=["Configuration"])
        ...
        @self.app.put("/config", response_model=ConfigurationResponse, tags=["Configuration"])
        async def update_configuration(config_update: ConfigurationUpdate):
            """Update agent configuration."""
            try:
                return self.controller.update_configuration(config_update.dict(exclude_unset=True))
        ...
        @self.app.get("/agent/status", response_model=AgentStatusResponse, tags=["Agent"])
    ...
    def start(self):
        ...
        self.logger.info(f"API server started on http://{self.api_host}:{self.api_port}")
        self.logger.info(f"API documentation available at http://{self.api_host}:{self.api_port}/docs")
...
def create_api_server(agent_process=None, 
                     config=None, 
                     data_dir=None, 
                     snmp_endpoint=None,
                     api_host="127.0.0.1",
                     api_port=8080,
                     cors_enabled=True,
                     cors_origins=None) -> SNMPAgentAPIServer:
