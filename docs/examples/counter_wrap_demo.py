...
"""
    ...
    try:
        ...
        print(
            "Time | Interface 1 (10M) | Interface 2 (100M) | Interface 3 (1G) | Interface 4 (10G)"
        )
        print("-" * 85)
        ...
        for i in range(20):  # Monitor for 20 iterations
                ...
                try:
                    result = subprocess.run(
                        [
                            ...
                            "1",
                            "127.0.0.1:11617",
                            f"1.3.6.1.2.1.2.2.1.10.{interface}",
                        ],
                        capture_output=True,
                        text=True,
                        timeout=2,
            ...
            # Format output
            print(
                f"{current_time:4.1f}s | {values[0]:>13,} | {values[1]:>14,} | {values[2]:>11,} | {values[3]:>12,}"
            )
            ...
            for j, value in enumerate(values):
                if i > 0 and value < 1000000:  # Likely wrapped
                    ...
                    print(
                        f"      *** WRAP DETECTED on Interface {j+1} ({speed_names[j]}) ***"
                    )
...
def show_counter_math():
    ...
    for name, bits_per_sec in speeds.items():
        ...
        print(
            f"         Wrap time: {seconds_to_wrap:>7.1f} seconds ({minutes_to_wrap:.1f} minutes)"
        )
        print()
