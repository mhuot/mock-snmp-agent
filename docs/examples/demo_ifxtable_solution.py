...
from src.behaviors.ifxtable_config import load_ifxtable_configuration
...
def demo_basic_functionality():
    ...
    for idx, interface_def in sorted(simulator.interfaces.items()):
        ...
        print(
            f"              Admin/Oper Status: {interface_def.admin_status.name}/{interface_def.oper_status.name}"
        )
        print(f"              Link Flap Enabled: {interface_def.link_flap_enabled}")
        if interface_def.link_flap_enabled:
            print(
                f"                → Flap every {interface_def.link_flap_interval}s for {interface_def.link_down_duration}s"
            )
        print()
    ...
    while time.time() - start_time < 10:
        ...
        for event in recent_events:
            event_id = (
                f"{event.interface_index}_{event.event_type.value}_{event.timestamp}"
            )
            if event_id not in events_shown:
                print(
                    f"    🔔 Event: Interface {event.interface_index} - {event.event_type.value}"
                )
...
def demo_snmp_integration():
    ...
    try:
        from src.behaviors.ifxtable_config import IfXTableConfigLoader
