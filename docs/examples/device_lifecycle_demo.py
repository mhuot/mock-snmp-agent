...
from src.state_machine.core import DeviceStateMachine
from src.state_machine.device_types import (
...
class DeviceLifecycleDemo:
    ...
    def _on_state_change(self, old_state: str, new_state: str, transition) -> None:
        ...
        print(
            f"   Time in previous state: {transition.duration_in_previous_state:.1f}s"
        )
    ...
    def show_current_state_details(self) -> None:
        ...
        print(
            f"  Duration: {current_state_def.duration_seconds}s"
    ...
    def demonstrate_manual_transitions(self) -> None:
        ...
        for next_state in current_state_def.next_states[
            :2
        ]:  # Limit to first 2 for demo
            print(f"\nTesting transition to '{next_state}'...")

            if self.state_machine.transition_to(
                next_state, reason=f"Demo transition to {next_state}"
            ):
                ...
                if current_state_def and current_state_def.next_states:
                    ...
                    self.state_machine.transition_to(
                        back_state, reason="Demo return transition"
                    )
                    time.sleep(1)
    ...
    def show_statistics(self) -> None:
        ...
        print(
            f"Auto Transitions: {'Enabled' if stats['auto_transitions_enabled'] else 'Disabled'}"
        )
    ...
    def show_state_history(self) -> None:
        ...
        for i, transition in enumerate(reversed(history), 1):
            ...
            print(f"     Reason: {transition['reason']}")
            print(
                f"     Duration in previous state: {transition['duration_in_previous_state']:.1f}s"
            )
            print()
