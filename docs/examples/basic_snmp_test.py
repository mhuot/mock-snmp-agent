...
def test_snmp_getnext():
    ...
    cmd = ["snmpgetnext", "-v2c", "-c", "public", "127.0.0.1:11611", "1.3.6.1.2.1.1"]
...
def test_different_oids():
    ...
    for oid, description in oids:
        ...
        if success:
            print(
                f"✓ {description}: {stdout.strip().split('=')[1].strip() if '=' in stdout else 'N/A'}"
            )
            success_count += 1
        else:
...
def test_snmp_walk():
    ...
    cmd = ["snmpwalk", "-v2c", "-c", "public", "127.0.0.1:11611", "1.3.6.1.2.1.1"]
