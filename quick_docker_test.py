...
"""
        ...
        delay_container = subprocess.Popen([
            "docker", "run", "--rm", "-d",
            "--name", "mock-snmp-delay-test",
            "-p", "11612:11612/udp",
            "mock-snmp-minimal",
            "mock-snmp-agent", "--delay", "500", "--port", "11612", "--host", "0.0.0.0"
        ], capture_output=True, text=True)
        ...
        if delay_container.returncode == 0:
            ...
            delay_test = subprocess.run([
                "snmpget", "-v2c", "-c", "public", "-t", "5",
                "localhost:11612", "1.3.6.1.2.1.1.1.0"
            ], capture_output=True, text=True)
            end_time = time.time()
            ...
            subprocess.run(["docker", "stop", delay_container_id], 
        ...
        # Cleanup
        subprocess.run(["docker", "stop", "mock-snmp-test"], 
                     capture_output=True, timeout=5)
        subprocess.run(["docker", "stop", "mock-snmp-delay-test"], 
                     capture_output=True, timeout=5)
...
def check_prerequisites():
    ...
    try:
        ...
        print("Install with: brew install net-snmp (macOS) or apt-get install snmp-utils (Linux)")
        return False
