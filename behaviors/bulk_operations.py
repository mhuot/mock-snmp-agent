...
class BulkOperationSimulator:
    ...
    def create_interface_table(self, num_interfaces: int = None) -> Dict[str, List]:
        ...
        interface_types = [
            "ethernet-csmacd", "gigabitEthernet", "tunnel", 
            "loopback", "ppp", "frameRelay"
        ]
    ...
    def simulate_getbulk_operation(
        ...
        if random.random() < self.config.failure_probability:
            ...
            return False, {
                "error": "genErr",
                "reason": "Simulated operation failure"
            }
...
if __name__ == "__main__":
    ...
    for i in range(5):
        success, result = simulator.simulate_getbulk_operation(
            "1.3.6.1.2.1.2.2.1.1.1",  # Start from ifIndex.1
            max_repetitions=10
        )
        
        if success:
            print(f"Operation {i+1}: ✓ {result['entry_count']} entries, "
