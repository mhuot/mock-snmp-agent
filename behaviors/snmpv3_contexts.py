...
class SNMPv3ContextManager:
    ...
    def _setup_default_contexts(self):
        ...
        self.add_context(ContextConfig(
            name="",
    ...
    def add_context(self, context: ContextConfig):
        ...
        self.logger.info(f"Added SNMPv3 context: '{context.name}' - {context.description}")
    ...
    def get_context_value(
        ...
        if user and context.allowed_users and user not in context.allowed_users:
            self.logger.warning(f"User '{user}' denied access to context '{context_name}'")
            return None
        
        # Check restricted OIDs
        for restricted_prefix in context.restricted_oids:
            if oid.startswith(restricted_prefix):
                self.logger.warning(f"OID '{oid}' restricted in context '{context_name}'")
                return None
    ...
    def _generate_context_value(self, context_name: str, oid: str, base_value: str) -> str:
...
if __name__ == "__main__":
    ...
    for ctx in context_mgr.list_contexts():
        ...
        print(f"  OIDs: {ctx['oid_count']}, Users: {ctx['user_count']}, Restricted: {ctx['restricted_count']}")
        print()