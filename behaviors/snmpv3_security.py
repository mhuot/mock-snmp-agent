...
class SNMPv3SecuritySimulator:
    ...
    def generate_time_window_failure(self, oid: str) -> Optional[str]:
        ...
        if not self.should_trigger_failure('time_window', self.config.time_window_failure_rate):
            return None
    ...
    def generate_privacy_failure(self, oid: str) -> Optional[str]:
        ...
        if not self.should_trigger_failure('privacy', self.config.decryption_error_rate):
            return None
    ...
    def generate_security_failures(self, base_oids: List[str]) -> List[str]:
        ...
        if failure_entries:
            self.logger.info(f"Generated {len(failure_entries)} security failure entries")
