...
class SNMPv3SecurityDemo:
    ...
    def build_snmp_command(self, config_name: str, oid: str, operation: str = 'get') -> List[str]:
    ...
    def test_security_configuration(self, config_name: str, expected_to_succeed: bool = True) -> Dict:
    ...
    def test_all_security_scenarios(self) -> Dict:
        ...
        basic_cmd = ["snmpget", "-v2c", "-c", "public", "-t", "2", "-r", "1", 
                    self.endpoint, "1.3.6.1.2.1.1.1.0"]
        ...
        if not success:
            ...
            print("  python mock_snmp_agent.py --config config/snmpv3_security_test.yaml")
    ...
    def generate_test_report(self, results: Dict) -> None:
        ...
        for scenario in results['test_scenarios']:
            ...
            print(f"{status} {config_name:15s} - {success_rate:5.1f}% success "
        ...
        print(f"\nSecurity Failure Analysis:")
        print(f"- Time window violations: Detected by failed requests with time window errors")
        print(f"- Authentication failures: Detected by wrong user/credentials rejections")
